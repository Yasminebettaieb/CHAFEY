        ...
        for p in range(0, net.module._num_prototypes):
            ...
            if c_weight>0:
                ...
                if found_max > 0.5:
                    ...
                    print(img_to_open)
                    filename = os.path.join(save_path.replace("\\", "/"), 'p%s_%s_%s_%s_rect.png'%(str(p),str(imglabel),str(round(found_max, 2)),str(img_to_open.replace("\\", "/").split('/')[-1].split('.jpg')[0].split('.png')[0])))
